...
class ProductVariationAttributeMapperTest extends CommerceKernelTestBase {
   ...
   */
  public function testSelectWithNoAttributes() {
   ...
   */
  public function testSelectWithWithOneAttribute() {
    $product = $this->generateThreeByTwoScenario();
   ...
   */
  public function testSelectWithWithTwoAttributes() {
    $product = $this->generateThreeByTwoScenario();
    $variations = $product->getVariations();
  ...
  /**
   * Tests the prepareAttributes method.
   */
  public function testPrepareAttributes() {
    ...
    $this->assertCount(2, $color_attribute->getValues());
    ...
    $this->assertCount(3, $size_attribute->getValues());
    ...
    $this->assertEquals('select', $size_attribute->getElementType());
    $this->assertTrue($size_attribute->isRequired());
    $this->assertCount(2, $size_attribute->getValues());
    $this->assertFalse(in_array('Large', $size_attribute->getValues()));
  }
  ...
  public function testPrepareAttributesOptional() {
    ...
    $this->assertNotCount(3, $disk2_attribute->getValues(), 'Out of the three available attribute values, only the one used is returned.');
    // There are two values. Since this is optional there is a "_none" option.
    $this->assertCount(1, $disk2_attribute->getValues());
    $this->assertTrue(isset($disk2_attribute->getValues()['_none']));
    ...
    $this->assertNotCount(3, $disk2_attribute->getValues(), 'Out of the three available attribute values, only the one used is returned.');
    // There are two values. Since this is optional there is a "_none" option.
    $this->assertCount(2, $disk2_attribute->getValues());
    $this->assertTrue(isset($disk2_attribute->getValues()['_none']));
  }
  ...
  public function testMutuallyExclusiveAttributeMatrixTwoByTwobyTwo() {
    ...
    $this->assertNotCount(3, $disk2_attribute->getValues(), 'Out of the three available attribute values, only the one used is returned.');
    $this->assertCount(1, $disk2_attribute->getValues());
    $this->assertTrue(in_array('2TB', $disk2_attribute->getValues()), 'Only the one valid Disk 2 option is available.');
    ...
    $this->assertTrue($disk1_attribute->isRequired());
    $this->assertNotCount(3, $disk1_attribute->getValues(), 'Out of the three available attribute values, only the one used is returned.');
    $this->assertCount(2, $disk1_attribute->getValues());
    ...
    // There should only be one Disk 2 option, since the other 8GB RAM option
    // has a Disk 1 value of 2TB.
    $this->assertCount(1, $disk2_attribute->getValues(), print_r($disk2_attribute->getValues(), TRUE));
  }
  ...
  public function testThreeAttributesSixVariations() {
    ...
    $color_attribute = $attributes['attribute_color'];
    $this->assertCount(2, $color_attribute->getValues());
    $this->assertTrue(in_array('Black', $color_attribute->getValues()));
    $this->assertTrue(in_array('Blue', $color_attribute->getValues()));
    $pack_attribute = $attributes['attribute_pack'];
    $this->assertCount(1, $pack_attribute->getValues());
    $this->assertTrue(in_array('1', $pack_attribute->getValues()));
    // The resolved variation is Small -> Black -> 1, cannot choose 20 for the
    // pack size, since that is Small -> Blue -> 20.
    $this->assertFalse(in_array('20', $pack_attribute->getValues()));
    ...
    $attributes = $this->mapper->prepareAttributes($selected_variation, $product->getVariations());
    $size_attribute = $attributes['attribute_size'];
    $this->assertCount(3, $size_attribute->getValues(), print_r($size_attribute->getValues(), TRUE));
    $color_attribute = $attributes['attribute_color'];
    $this->assertCount(2, $color_attribute->getValues());
    $this->assertTrue(in_array('Green', $color_attribute->getValues()));
    $this->assertTrue(in_array('Red', $color_attribute->getValues()));
    $pack_attribute = $attributes['attribute_pack'];
    $this->assertCount(1, $pack_attribute->getValues());
    // The resolved variation is Medium -> Green -> 100, cannot choose 200 for
    // the pack size, since that is Medium -> Red -> 200.
    $this->assertTrue(in_array('100', $pack_attribute->getValues()));
    $this->assertFalse(in_array('200', $pack_attribute->getValues()));
  }
